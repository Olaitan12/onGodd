...
final Function login = (BuildContext context, String username, String password)  {
    return (Store<AppState> store) async {
        ...
        if (response['status']) {
            store.dispatch(new UserLoginSuccess(new User(response['token'],response['details']['id'],response['details']['email'])));
...
final Function selectUserCompany = (BuildContext context, String company)  {
    return (Store<AppState> store) async {
         
            store.dispatch({ "type": "SELECT_COMPANY", "payload": company });
